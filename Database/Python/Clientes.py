...
def create(_name:str ,_cpf:str ,_email:str ,_phone:str):
    try:
        # Verificar todos os campos "unique", Impossibilitar a criação caso valores duplicados.
        ...
        # Criação do cliente
        cursor.execute('INSERT INTO Customer (name, cpf , email, phone) VALUESS (?,?,?,?)' , (_name,_cpf,_email,_phone));
        connection.commit();
...
def findByCPF(_cpf:str):
    try:
        ...
        if( customer == None ):
            ...
            {'status': 200 , 'message' : 'Customer found' , 'data' : customer};
